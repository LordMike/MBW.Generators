...
internal static class Gen
{
    ...
    private static MethodDeclarationSyntax CreateShell(PlannedMethod pm, bool isAsync)
    {
        ...
        var ret = SyntaxFactory.ParseTypeName(pm.Signature.ReturnType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat));
    ...
    private static ParameterSyntax BuildExtensionThisParam(PlannedMethod pm, string receiverName)
    {
        ...
        var recvType = SyntaxFactory.ParseTypeName(m.ContainingType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat));
        ...
        return SyntaxFactory.Parameter(SyntaxFactory.Identifier(receiverName)).WithType(recvType).WithModifiers(SyntaxFactory.TokenList(mods));
    }
        ...
        ComputeExtensionBits(PlannedMethod pm)
    {
        ...
        var target = SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.IdentifierName(recv),
    ...
    private static ThrowStatementSyntax BuildThrow(PlannedMethod pm)
        => SyntaxFactory.ThrowStatement(
            SyntaxFactory.ObjectCreationExpression(
                    SyntaxFactory.ParseTypeName(pm.ExceptionType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat)))
    ...
    private static MethodDeclarationSyntax BuildNonTrySync(PlannedMethod pm)
    {
        ...
        var tryCall = SyntaxFactory.InvocationExpression(target)
            .WithArgumentList(SyntaxFactory.ArgumentList(SyntaxFactory.SeparatedList(RenderTryArgs(pm, isAsync: false))));

        return decl.WithBody(SyntaxFactory.Block(
            SyntaxFactory.IfStatement(tryCall, SyntaxFactory.Block(SyntaxFactory.ReturnStatement(SyntaxFactory.IdentifierName(outParam)))),
            BuildThrow(pm)
        ));
    }
    ...
    private static MethodDeclarationSyntax BuildNonTryAsync(PlannedMethod pm)
    {
        ...
        var awaitCall = SyntaxFactory.AwaitExpression(SyntaxFactory.InvocationExpression(target)
            .WithArgumentList(SyntaxFactory.ArgumentList(SyntaxFactory.SeparatedList(RenderTryArgs(pm, isAsync: true)))));
        ...
        var declLocal = SyntaxFactory.LocalDeclarationStatement(
            SyntaxFactory.VariableDeclaration(SyntaxFactory.IdentifierName("var"))
                .WithVariables(SyntaxFactory.SeparatedList([SyntaxFactory.VariableDeclarator(t).WithInitializer(SyntaxFactory.EqualsValueClause(awaitCall))])));

        var ok = SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.IdentifierName(t),
            SyntaxFactory.IdentifierName("Item1"));
        var val = SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.IdentifierName(t),
            SyntaxFactory.IdentifierName("Item2"));

        return decl.WithBody(SyntaxFactory.Block(
            declLocal,
            SyntaxFactory.IfStatement(ok, SyntaxFactory.Block(SyntaxFactory.ReturnStatement(val))),
            BuildThrow(pm)
        ));
    }
    ...
    private static ParameterSyntax RenderParameter(IParameterSymbol p)
    {
        ...
        if (p.IsParams)
            param = param.WithModifiers(SyntaxFactory.TokenList(param.Modifiers.Concat([SyntaxFactory.Token(SyntaxKind.ParamsKeyword)])));
        if (p.HasExplicitDefaultValue)
        {
            var equals = SyntaxFactory.EqualsValueClause(GenerationHelpers.ToCSharpLiteralExpression(p.ExplicitDefaultValue));
